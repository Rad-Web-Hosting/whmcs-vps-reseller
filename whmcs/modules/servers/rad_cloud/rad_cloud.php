	...
	// If we get the rad server in configoption1, we will make an API call and load other fields
		...
		if(!empty($data['inhouse_billing'])){
			...
			foreach($data['servergroups'] as $k => $v){
				...
				$tmp_default_node_grp[$k] = $k.' - '.(!empty($v['sg_reseller_name']) ? $v['sg_reseller_name'] : $v['sg_name']);
...
function rad_cloud_ChangePassword($params) {
	...
	$post_vps['ips'] = VPHP::count($post_vps['ips']);
	$post_vps['ips6'] = VPHP::count($post_vps['ips6']);
	$post_vps['ips6_subnet'] = VPHP::count($post_vps['ips6_subnet']);
	$post_vps['ips_int'] = VPHP::count($post_vps['ips_int']);
	$post_vps['editvm'] = 1;
...
function rad_cloud_ChangePackage($params) {
	...
	if(is_array($post_vps['ips'])){
		$post_vps['ips'] = VPHP::count($post_vps['ips']);
	}
	if(is_array($post_vps['ips6'])){
		$post_vps['ips6'] = VPHP::count($post_vps['ips6']);
	}
	if(is_array($post_vps['ips6_subnet'])){
		$post_vps['ips6_subnet'] = VPHP::count($post_vps['ips6_subnet']);
	}
	if(is_array($post_vps['ips_int'])){
		$post_vps['ips_int'] = VPHP::count($post_vps['ips_int']);
	}
...
class VirtCloud_Curl {
	...
	static function action($params, $action, $post = array()){
